...
GPD_LL_sigR <- function(par, cat){
  ...
  sig <- numeric(length(cat$mag))
  sig[cat$RD_Y > (735000 - 0.6*cat$RD_X)] <- sigma1
  sig[cat$RD_Y <= (735000 - 0.6*cat$RD_X)] <- sigma2
  if((all(sig>0))){
    if(abs(xi)<1e-10){
      k<-sum(cat$mag/sig)
      return(-sum(log(sig))-k)
    }
    else{
      if(all(1+xi*(cat$mag/sig)>0)){
        l<-log(1+xi*(cat$mag/sig))
        #return(l)
        return(-sum(log(sig))-(1+1/xi)*(sum(l)))
      }  
      else{
        return(-1e6)
      }
