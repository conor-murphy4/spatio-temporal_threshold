...
Geo <- read.csv("STOR-i/PhD/Projects/Spatio-temporal/Data/Geophones/Geophone data/Geophone_rect.csv", header=T)
...
par(mfrow=c(2,3))
plot(Gron_outline$X, Gron_outline$Y)
points(Geo_poly$X, Geo_poly$Y)
text(loc1$X, loc1$Y, "1", cex=2)
text(loc2$X, loc2$Y, "2", cex=2)
text(loc3$X, loc3$Y, "3", cex=2)
text(loc4$X, loc4$Y, "4", cex=2)

plot(year, log(V_int), type='l', main="Integrated V", lwd=2)
par(mfrow=c(1,1))
plot(year, log(V_loc1),  lwd=2, type='l', ylim = c(12, 20))
